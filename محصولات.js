...
/**
 * سیستم چت آنلاین
 * شامل نمایش/مخفی کردن پنل چت، ارسال و دریافت پیام، و شبیه‌سازی چت با پشتیبان
 */
function setupChatSystem() {
    ...
    // پاسخ‌های از پیش تعیین شده پشتیبان
    const supportResponses = [
        "سلام! چطور می‌تونم کمکتون کنم؟",
        "بله، ما ارسال به تمام نقاط کشور داریم. هزینه ارسال برای خرید‌های بالای 500 هزار تومان رایگان است.",
        "محصولات ما دارای ضمانت اصالت و گارانتی تعویض 7 روزه هستند.",
        "برای مشاهده راهنمای سایز می‌تونید به بخش راهنمای سایز در فوتر سایت مراجعه کنید.",
        "بله، امکان پرداخت در محل برای شهرهای تهران، مشهد، اصفهان، شیراز و تبریز وجود داره.",
        "موجودی انبار به صورت لحظه‌ای بروزرسانی میشه. اگر محصولی رو مشاهده می‌کنید، یعنی در انبار موجود است.",
        "برای تعویض محصول کافیه با شماره 09900859556 تماس بگیرید تا همکاران ما راهنمایی‌تون کنند.",
        "بله، ما امکان ارسال عکس‌های بیشتر از محصول رو داریم. لطفاً نام محصول مورد نظرتون رو بفرمایید.",
        "ممنون از پیام شما. همکاران ما در اسرع وقت پیگیری خواهند کرد."
    ];
    ...
    if (clearChat) {
        clearChat.addEventListener('click', () => {
            // حفظ پیام اولیه سیستم
            const systemMessages = chatBody.querySelectorAll('.chat-message.system');
            chatBody.innerHTML = '';
            
            // افزودن جداکننده تاریخ
            const dateDiv = document.createElement('div');
            dateDiv.className = 'chat-date-separator';
            dateDiv.innerHTML = '<span>امروز</span>';
            chatBody.appendChild(dateDiv);
            
            // افزودن مجدد پیام سیستم
            systemMessages.forEach(msg => {
                chatBody.appendChild(msg.cloneNode(true));
            });
    ...
    function sendChatMessage() {
        ...
        if (messageText) {
            ...
            // شبیه‌سازی تایپ پشتیبان
            setTimeout(() => {
                typingIndicator.style.display = 'none';
                
                // انتخاب یک پاسخ تصادفی
                const randomIndex = Math.floor(Math.random() * supportResponses.length);
                const response = supportResponses[randomIndex];
                
                // افزودن پاسخ پشتیبان به چت
                addMessageToChat('agent', response);
            }, 1500 + Math.random() * 1500); // زمان تایپ تصادفی بین 1.5 تا 3 ثانیه
        }
    }
    ...
    function addMessageToChat(sender, text) {
        ...
        chatBody.scrollTop = chatBody.scrollHeight; // اسکرول به آخرین پیام
    }
    
    // شبیه‌سازی یک پیام اولیه از پشتیبان بعد از بارگذاری صفحه
    setTimeout(() => {
        if (badgeVisible) {
            chatBadge.style.display = 'flex';
            
            // پیام خودکار از پشتیبان بعد از مدتی
            setTimeout(() => {
                if (!chatPopup.classList.contains('active')) {
                    const randomIndex = Math.floor(Math.random() * supportResponses.length);
                    addMessageToChat('agent', "سلام! می‌توانم کمکتون کنم؟ برای مشاوره و پاسخگویی به سوالات شما آماده‌ام.");
                }
            }, 15000); // 15 ثانیه بعد از بارگذاری صفحه
        }
    }, 3000);
    
    // تنظیم دکمه‌های اضافی 
    ...
    if (attachFileBtn) {
        attachFileBtn.addEventListener('click', () => {
            showNotification('قابلیت ارسال فایل در حال حاضر فعال نیست', 'info');
        });
    }
    
    if (sendEmojiBtn) {
        sendEmojiBtn.addEventListener('click', () => {
            showNotification('پنل ایموجی به زودی فعال خواهد شد', 'info');
        });
    }
    
    if (voiceChatBtn) {
        voiceChatBtn.addEventListener('click', () => {
            showNotification('امکان ارسال پیام صوتی به زودی اضافه خواهد شد', 'info');
        });
    }
}

// اجرای تنظیمات چت در زمان بارگذاری صفحه
document.addEventListener('DOMContentLoaded', () => {
    setupChatSystem();
});
/**
 * آزمایش سیستم اعلان و چت
 * این تابع باعث فعال‌سازی و آزمایش سیستم می‌شود
 */
function testNotificationAndChatSystem() {
    // آزمایش سیستم اعلان با انواع مختلف
    setTimeout(() => {
        showNotification('سیستم اعلان‌ها با موفقیت فعال شد', 'success', 4000);
    }, 1000);
    
    setTimeout(() => {
        showNotification('امکان ارسال فایل به زودی فعال خواهد شد', 'info', 4000);
    }, 2000);
    
    // فعال‌سازی دکمه‌های چت
    const attachFileBtn = document.getElementById('attach-file');
    const sendEmojiBtn = document.getElementById('send-emoji');
    const voiceChatBtn = document.getElementById('voice-chat-btn');
    
    // باز کردن اتوماتیک پنل چت بعد از مدتی
    setTimeout(() => {
        const chatToggle = document.getElementById('chat-toggle');
        if (chatToggle) {
            chatToggle.click();
    ...
    // تست ارسال پیام اتوماتیک در چت
    setTimeout(() => {
        const chatInput = document.getElementById('chat-input');
        const sendMessage = document.getElementById('send-message');
        
        if (chatInput && sendMessage) {
            chatInput.value = 'سلام، این یک پیام تست است';
            sendMessage.click();
        }
    }, 5500);
}

// اجرای کد تست بعد از بارگذاری کامل صفحه
window.addEventListener('load', function() {
    testNotificationAndChatSystem();
});
